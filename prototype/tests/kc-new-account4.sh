...
kc get users/$id -r $R | jq -c '{username, email, firstName, lastName, idp: .attributes.ns8_idp, idp_id: .attributes.ns8_idp_id, federationLink}'
...
conn.search('CN=Users,DC=ad,DC=dom,DC=test', '(sAMAccountName=e.u4)',
    attributes=['cn', 'displayName', 'mail', 'employeeType', 'employeeNumber', 'userAccountControl', 'pwdLastSet', 'userPrincipalName', 'whenCreated'])
...
api-cli run cluster/list-domain-users --data '{"domain":"ad.dom.test"}' | jq -c '.users[] | select(.user=="e.u4")'
...
echo "== nextcloud2"
runagent -m nextcloud2 podman exec --user www-data nextcloud-app php ./occ user:list --output=json 2>/dev/null </dev/null | jq -r 'to_entries[] | select(.value|test("e.u4")) | "\(.key)\t\(.value)"'
